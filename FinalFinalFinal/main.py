...
'''
match frame的时候，比较img[:,75:]
'''

print('-------selecting frames-------')
FF = Frame_selector()
FF.set_path('/Users/zhaosonglin/Documents/GitHub/EIE4512_pano_proj/videos/7.19_2.MOV')
...
for i in range(len(imgs)):
    ...
    if v > 200:
        if i == 0:
            img = motion_deblur.wiener_deblur_color(img, v, T, 0.1, mask1)
        else:
            img = motion_deblur.wiener_deblur_color(img, v, T, 0.1, mask2)
    imgs[i] = img
